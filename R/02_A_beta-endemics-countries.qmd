---
title: "A_Beta_endemics - Turnover with endemic species"
format: html
---

# Overview

Here we process the data on species native distribution filtering by species with 
    only one occurrence of native distribution per country. This was done in 
    order to reduce the effects of species with large distribution ranges in the
    the metrics of native turnover and NBT turnover
...
sparse_all_pa3 <- 
  sparse_all_pa2[, colnames(sparse_all_pa2) %in% spp_endemic_native$valid_name]
```
...
df_endemic_beta <- beta_types(presab = sparse_all_pa3, 
                          names.countries = names_countries) # calculating metrics
