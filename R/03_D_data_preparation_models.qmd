...
Joining name bearers and native data with Bio-Dem, Infra-museum, and native deficit and non-endemic name bearers proportions
...
Here we will model the number (counting) of total name bearers in each country. Countries with no name bearers in their museums and natural history collections are represented with NA. However the absence of primary types are an important information, and consist in a true absence, meaning that these countries host no name bearers. So, first we transformed the NA values in `type_richness` variable to 0. Then we standardized all the predictors to 0 mean and 1 variation unit.
...
Transforming endemic deficit and non-endemic name bearers proportions to fit in the model. Here we transform these proportions to fit it between zero and one, following [Ferrari and Cribari-Neto](https://www.ime.usp.br/~sferrari/beta.pdf)
...
Here we perform some other calculations to add the total name bearers in museums, countries and some simple proportions
